...
public class CMD implements TabExecutor {
        ...
        if (args.length > 0) {
            ...
            if (subArg.contains(arg.toLowerCase())) {
                if (arg.equalsIgnoreCase("help")) {
                    sender.sendMessage(Config.helpMsg);
                }
                if (arg.equalsIgnoreCase("reload")) {
                    ...
                    sender.sendMessage(PapiUtil.papi(null, Config.plugin.getConfig()
                ...
                if (arg.equalsIgnoreCase("open")) {
                    ...
                    PCPageGui pcGui = new PCPageGui(new PCBox(FICoreAPI.getPokeStorageManager().getPC((other == null ? player : other).getUniqueId()), page - 1));
                ...
                if (arg.equalsIgnoreCase("search")) {
                    ...
                    for (int i = 1; i < args.length; i++) {
                        ...
                        String k = split[0];
                        String a = split[1];
                        if (!Config.searchProperties.containsKey(k)) {
                            sender.sendMessage("无效参数: " + s);
                            return false;
                        }
                        map.put(Config.searchProperties.get(k), a);
                    }
                    Player player = (Player) sender;
                    if (map.isEmpty()) {
                        player.closeInventory();
                    ...
                    val pokemons = FICoreAPI.getPokeStorageManager().getPC(player.getUniqueId()).all();
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args[0].equalsIgnoreCase("search")) {
            ...
            if (!arg.contains(":"))
                return Config.searchProperties.keySet().stream().
                        filter(s -> s.startsWith(arg) && !list.contains(s)).map(s -> s + ":").
                        collect(Collectors.toList());
            String[] split = arg.split(":");
            if (!(sender instanceof Player)) return null;
            String key = split[0];
            return Config.searchProperties.get(key).onTabComplete(((Player) sender), split.length == 2 ? split[1] : "").
